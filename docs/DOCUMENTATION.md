...
## 🔐 DevSecOps (best-effort)

Ces commandes n'ajoutent pas de dépendances automatiquement: elles détectent les outils disponibles et affichent quoi installer si besoin.

- Scans dépendances/SAST/secrets: `./scripts/security_scan.sh`
- Scan filesystem / container (si Trivy dispo): `./scripts/container_scan.sh`

---

## 🎫 Tickets internes (v2)

- `GET /v2/tickets?limit=...`
- `POST /v2/tickets`
- `PATCH /v2/tickets/{ticket_id}` (assignation/workflow/historique stockés dans `payload` JSON + audit)
- `POST /v2/tickets/{ticket_id}/resolve`

Les actions critiques écrivent dans `audit_logs`.
