...
|------|-------------|----------|
| `/login` | Connexion utilisateur (SPA) | Form, error handling, auto-redirect |
| `/register` | Inscription nouvel utilisateur (SPA) | Multi-field form, validation client |
| `/calendar` | Calendrier principal (SPA) | FullCalendar, navbar |
| `/profile` | Gestion profil utilisateur (SPA) | Edit, password change, delete account |
| `/admin/users` | Admin panel (SPA) | User list, edit roles, delete |
...
├── /calendar ─────→ GET /auth/me, GET /events/
├── /profile ──────→ PUT /auth/me, POST /auth/change-password
└── /admin/users ──→ GET /users/, PUT /users/{id}, DELETE /users/{id}
